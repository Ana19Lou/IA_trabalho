...
public class DFS{
	...
	public static void createDFS(ArrayList<Integer> possibleRectangles,HashMap<Integer, ArrayList<Integer>> map,int n_rectangles, HashMap<Integer, ArrayList<Integer>> mapRec) {
		removeRectangles(n_rectangles, possibleRectangles, map, mapRec);
        DFSSolver dfsSolver = new DFSSolver(possibleRectangles.size(), possibleRectangles, mapRec,map);
        ArrayList<Integer> bestPath = new ArrayList<>();
        int min = Integer.MAX_VALUE;
        for(ArrayList<Integer> path: dfsSolver.allPaths) {
            if(min > path.size()) {
                min = path.size();
                bestPath = path;
            }
    ...
    public static void createInstance(int n_rectangles, Scanner in) {
        ...
        createDFS(possibleRectangles,map,n_rectangles,mapRec);
	}
